...
function autoBind(instance: object): void {
  ...
  Object.getOwnPropertyNames(proto).forEach(key => {
    ...
    if (key !== 'constructor' && typeof inst[key] === 'function') {
      inst[key] = (inst[key] as (...a: unknown[]) => unknown).bind(instance);
    }
  });
}
...
/* ── Media (one card) ───────────────────────────────── */
class Media {
  extra = 0; isBefore = false; isAfter = false; speed = 0;
  x = 0; width = 0; widthTotal = 0; scale = 0; padding = 0;
  ...
  program: any; plane: any; title: any; videoEl: HTMLVideoElement | null = null;
  ...
  constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius = 0, font }: any) {
    ...
    this.text = text; this.viewport = viewport; this.bend = bend;
  ...
  createShader() {
    ...
    const isVideo = /\.(mp4|webm|mov|ogg)$/i.test(this.image);
    if (isVideo) {
  ...
  update(scroll: { current: number; last: number }, direction: 'left' | 'right') {
    ...
    if (this.videoEl) (this as any)._texture.needsUpdate = true;
...
class App {
  ...
  renderer!: Renderer; gl!: any; camera!: Camera; scene!: Transform;
...
export default function CircularGallery({
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Scrollable image gallery. Drag or use arrow keys to navigate."
    />
  );
}
