...
export default function OptionWheel({
  ...
  const runFrame = useCallback((now: number) => {
    ...
    const cfg = cfgRef.current as {
      count: number; rowH: number; curve: number; tilt: number;
      blur: number; fade: number; minOpacity: number;
      side: string; loop: boolean; smoothing: number;
    };
    ...
    rafRef.current = (next === target) ? null : requestAnimationFrame(runFrame);
  }, []);
  ...
  useLayoutEffect(() => {
    /* Synchronously position all items before the first paint so they're
       never invisible. runFrame is then also called via applyTarget/startLoop
       for smooth animated transitions when props change. */
  ...
  /* Build style — only set color vars if props are explicitly supplied */
  const wheelStyle: React.CSSProperties & Record<string, string> = {
    '--ow-font-size': `${fontSize}rem`,
    '--ow-inset':     `${inset}px`,
  };
  if (textColor   != null) wheelStyle['--ow-text-color']   = textColor;
  if (activeColor != null) wheelStyle['--ow-active-color'] = activeColor;
